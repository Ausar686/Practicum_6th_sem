...
elseif optionStart == 5
    ...
    elseif optionPlot == 5
        if optionSubOptimal == 1
            plot(arrayT, arrayU2);
            if resultTime < t0 + constTimeLimit
                plot(resultT, resultU(2, :), 'r*');
            end
        else
            plot(resultT, resultU(2, :)); 
        end
        xlabel('t');
        ylabel('u2');
        legend;
    elseif optionPlot == 6
        if optionSubOptimal == 1
            plot(arrayT, arrayPsi1);
            if resultTime < t0 + constTimeLimit
                plot(resultT, resultPsi(1, :), 'r*');
            end
        else
            plot(resultT, resultPsi(1, :)); 
        end
        xlabel('t');
        ylabel('psi1');
        legend;
    elseif optionPlot == 7
        if optionSubOptimal == 1
            plot(arrayT, arrayPsi2);
            if resultTime < t0 + constTimeLimit
                plot(resultT, resultPsi(2, :), 'r*');
            end
        else
            plot(resultT, resultPsi(2, :)); 
        end
        xlabel('t');
        ylabel('psi2');
    elseif optionPlot == 8
        if optionSubOptimal == 1
            plot(arrayU1, arrayU2);
            if resultTime < t0 + constTimeLimit
                plot(resultU(1, :), resultU(2, :), 'r*');
            end
        else
            plot(resultU(1, :), resultU(2, :)); 
        end
        xlabel('u1');
        ylabel('u2');
        legend;
    elseif optionPlot == 9
        ...
        plot(resultX(1, :), resultX(2, :), x_plot1, y_plot1, resultX(1, end), resultX(2, end), 'y*', x_sup(1), x_sup(2), 'r*');
        xlabel('x1');
        ylabel('x2');
        legend('Граница Х0', 'Граница Х1', 'Оптимальная траектория', 'Вектор -psi(t1)', 'Конечная точка', 'Численно найденная опорная точка');
        disp(abs(spf - scp));
    else
